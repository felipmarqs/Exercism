...
def get_leftover_of_bills(budget:float, denomination:int) -> int:
    """
...
def exchangeable_value(budget:float, exchange_rate:float, spread:int, denomination:int):
    ...
    current_exchange_rate = exchange_rate * ((100 + spread) / 100)
    current_value = exchange_money(budget,current_exchange_rate)
    return get_number_of_bills(current_value, denomination) * denomination